...
async function main(): Promise<void> {
  for (const [index, category] of CATEGORIES.entries()) {
    await prisma.category.upsert({
      ...
      // Name and ordering are safe to refresh; the active flag remains an
      // administrator decision and is not overwritten by a reseed.
  ...
  for (const [index, group] of RESPONSIBLE_GROUPS.entries()) {
    await prisma.responsibleGroup.upsert({
      ...
      create: {
        ...
        authorityName: group.authorityName ?? group.name,

...
export function rulesText(): string {
  ...
  return [
    ...
    '',
    'При работе с сообщениями, поданными через бот, не применяются положения Федерального закона «О порядке рассмотрения обращений граждан Российской Федерации» от 02.05.2006 №59-ФЗ.',
    '',
    ...
    '',
    '5. Текст сообщения не позволяет определить суть вопроса, отсутствует адрес проблемы.',
    '',

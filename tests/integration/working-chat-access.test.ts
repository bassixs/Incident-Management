...
describeIntegration('automatic access in configured work chats', () => {
  ...
  it('runs assignment, answering and approval through real routers for employees without manual roles', async () => {
    ...
    const draft = (await h.services.sessions.find(86002n, TEST_CHATS.sector))!.data as any;
    await click(TEST_CHATS.sector, incidentCallback('action-confirm', row.id, draft.confirmation.token), 86002);
    ...
    const approval = (await h.services.sessions.find(86003n, TEST_CHATS.review))!.data as any;
    await click(TEST_CHATS.review, incidentCallback('action-confirm', row.id, approval.confirmation.token), 86003);
  ...
  it('existing members get suitable info and working commands on their first message without rejoining', async () => {
    ...
    expect(lastText()).toContain('Хозяйственная группа'); expect(lastText()).not.toMatch(/срок|шаблон|типов/i);

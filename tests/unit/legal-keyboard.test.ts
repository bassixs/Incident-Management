...
describe('legal document keyboards', () => {
  ...
  it('shows three HTTPS documents and a separate continue action', () => {
    const keyboard = legalDocumentsKeyboard(links, { showContinue: true });
    const urls = keyboard
      .flat()
      .filter((button) => button.type === 'link')
      .map((button) => ('url' in button ? button.url : undefined));
    expect(urls).toEqual(Object.values(links));
    expect(parseCallbackPayload(callbackOf(keyboard, 'Продолжить'))).toEqual({
      kind: 'user',
      action: 'legal-continue',
    });
  });

...
describe('executor controls by incident stage', () => {
  it.each([
    ['ASSIGNED', ['Взять в работу', 'Подготовить ответ', 'Использовать шаблон', 'Вернуть на перераспределение', 'Освободить обращение']],
    ['IN_PROGRESS', ['Подготовить ответ', 'Использовать шаблон', 'Вернуть на перераспределение', 'Освободить обращение']],

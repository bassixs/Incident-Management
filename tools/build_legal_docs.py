...
def build_policy() -> Path:
    ...
    add_heading(doc, "4. Состав обрабатываемых данных")
    add_body(doc, "Оператор может обрабатывать:")
    for item in (
        "ФИО Пользователя и контактный номер телефона;",
        "сведения об обращении: округ возникновения проблемы, текст и предоставленные при необходимости фотографии, геолокацию или точный адрес;",
        "технические сведения, автоматически передаваемые мессенджером MAX и необходимые для работы Чат-бота;",
        "сведения о ходе и результате рассмотрения обращения.",
    ):
        add_bullet(doc, item)
    add_body(doc, "Чат-бот не запрашивает округ проживания. Пользователь не должен сообщать избыточные данные, не относящиеся к обращению.")
    ...
    p = doc.add_paragraph()
    p.paragraph_format.first_line_indent = Cm(0)
    r = set_run_font(p.add_run("[ДО ПУБЛИКАЦИИ: обеспечить удаление из базы, журналов и резервных копий либо указать реальный срок хранения.]"), bold=True)
    shade_run(r)
